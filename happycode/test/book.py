#!/usr/bin/python
# -*- coding: utf-8 -*-
# @Date    : 2016-12-20 18:40:20
# @Author  : Yunyu2019 (yunyu2010@yeah.net)
# @Link    : 
# @descp   : The document description

import json
import codecs
import requests
import ConfigParser

class Book(object):
	"""docstring for Book"""
	def __init__(self,headers):
		self.headers=headers
		self.getConfig()
		self.source='http://{0}:{1}/{2}'.format(self.config['Site']['url'],self.config['Site']['port'],'api/book')

	def getConfig(self):
		cf = ConfigParser.RawConfigParser()
		cf.read('config.ini')
		sectiones=cf.sections()
		config={}
		for i in sectiones:
			opts=cf.options(i)
			temp={}
			for j in opts:
				temp[j]=cf.get(i,j)
			config[i]=temp
		self.config=config

	def getlist(self):
		print 'book list'
		url='{0}/{1}'.format(self.source,'list')
		req=requests.get(url,headers=self.headers)
		cont=req.content
		print cont

	def add(self):
		print 'book add'
		name=raw_input('please input the book name:')
		target=raw_input('please input the book target:')
		descp=raw_input('please input the book description:')
		data={
			'Book[name]':name,
			'Book[target]':target,
			'Book[descp]':descp
		}
		url='{0}/{1}'.format(self.source,'add')
		req=requests.post(url,headers=self.headers,data=data)
		print req.content

	def save(self):
		print 'book save'
		bookid=raw_input('please input the book id:')
		name=raw_input('please input the book name:')
		target=raw_input('please input the book target:')
		descp=raw_input('please input the book description:')
		data={
			'Book[id]':bookid,
			'Book[name]':u'{0}'.format(name),
			'Book[target]':target,
			'Book[descp]':u'{0}'.format(descp)
		}
		url='{0}/{1}'.format(self.source,'save')
		req=requests.post(url,headers=self.headers,data=data)
		print req.content

	def getOne(self):
		print 'book info'
		bookid=raw_input('please input the book id:')
		data={
			'bookid':bookid
		}
		url='{0}/{1}'.format(self.source,'getbook')
		req=requests.post(url,headers=self.headers,data=data)
		print req.content

	def delete(self):
		print 'book delete'
		bookid=raw_input('please input the book id:')
		data={
			'bookid':bookid
		}
		url='{0}/{1}'.format(self.source,'delete')
		req=requests.post(url,headers=self.headers,data=data)
		print req.content


if __name__ == '__main__':
	headers={
	'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.143 Safari/537.36'
	}
	with codecs.open('token.txt','r',encoding="utf-8") as fp:
		for line in fp.readlines():
			token=line
	headers['X-token']=token
	book=Book(headers)
	book.getlist()